...
class GatedCombiner(nn.Module):
    def __init__(self, struct_emb_dim, gnn_emb_dim, output_dim):
        ...
        self.gate = nn.Sequential(
            nn.Linear(struct_emb_dim + gnn_emb_dim, output_dim),
            nn.Sigmoid()
        )
    ...
    def forward(self, struct_emb, gnn_emb):
        ...
        if struct_emb.size(-1) != self.struct_emb_dim:
            raise ValueError(f"Expected struct_emb dimension {self.struct_emb_dim}, got {struct_emb.size(-1)}")
        if gnn_emb.size(-1) != self.gnn_emb_dim:
            raise ValueError(f"Expected gnn_emb dimension {self.gnn_emb_dim}, got {gnn_emb.size(-1)}")
...
class MGAT(nn.Module):
    ...
    def __init__(self, num_hidden, in_dim, hidden_dim, output_dim, heads, activation, aggregation='concat',
...
class MLStruct(torch.nn.Module):
    def __init__(self, edge_dim, node_dim, phi_dim, n_layers, beta=1.0, dropout=0.2,
        ...
        for _ in range(n_layers):
            ...
            g_phi = torch.nn.Sequential(
                torch.nn.Linear(1, phi_dim),
                nn.ReLU(), 
                nn.Linear(phi_dim, node_dim) 
            )
    ...
    def random_fill(self, mat, dtype=torch.float32):
        mat.mul_(1 + F.dropout(
            torch.full_like(mat, self.eps),
            p=self.f_dropout
        ))
        return mat
    ...
    def forward(self, graphs, edge_w=False):
        ...
        for l_id in range(self.n_layers):
            ...
            try:
                ...
                if 'edge_weights' in locals(): del edge_weights
...
class MultiGraph(nn.Module):
    def __init__(self, graph, dropout, device="cpu", eps=1e-8, **args):
        ...
        self.gnn_model = MGAT(self.num_hidden, self.input_dim, self.hidden_dim, self.output_dim, self.heads, self.activation, self.aggregation, self.dropout, self.attn_dropout, residual=self.residual)
        ...
        if self.struct:
            ...
            for _ in range(self.n_layers):
                ...
                if self.gnn_dim != self.output_dim:
                    self.gnn_projections.append(nn.Sequential(
    ...
    def forward(self, graphs, p, g_supra):
        ...
        if self.struct:
            ...
            for l_id in range(self.n_layers):
                struct_emb = node_embeddings['struct_feats'][l_id] if 'struct_feats' in node_embeddings else None
                gnn_emb = node_embeddings['layer_wise'][l_id] if 'layer_wise' in node_embeddings else None
